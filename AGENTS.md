...
- `editableApiModelId?: boolean` — opt-in pencil editor, only for providers that accept arbitrary upstream ids (e.g. BytePlus, which addresses a model either by its public id or by a custom `ep-...` inference endpoint). Ignored when `aggregated`. `isApiModelIdEditable()` in `src/models/index.ts` is the single source of truth; do not re-derive the rule in a UI or a migration.
- Stored overrides live in `apiModelIdOverrides[providerId][modelId]`, and `resolveApiModelId` prefers them over the catalog id **unconditionally**. An override on a pairing that is not editable therefore keeps rewriting every request while the UI shows no pencil and no "Modified" badge. `pruneApiModelIdOverrides()` runs on every load to enforce the invariant "an override exists only where a user could have set it" — revoking `editableApiModelId` discards existing overrides for that pairing by design, rather than leaving them live and invisible.
- `aggregated?: boolean` — the provider routes the model to multiple upstream ids internally, keyed by mode (DashScope Wan 2.7 / HappyHorse 1.1; DashScope voice) or by a param (APIMart GPT Image 2.5's `variant`). Use it whenever the catalog `apiModelId` is a display-only umbrella that is not callable on its own — do NOT fork one catalog entry per upstream build. Routing stays hard-coded in the provider; the catalog only marks it. Locks the id editor. Must not also set `editableApiModelId`.
